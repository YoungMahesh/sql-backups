...
/**
 * GET /api/postgres/backups/[id]/tables/[table]/rows
 *
 * Streams the gzipped dump from S3, runs the parser, and returns the first
 * `ROW_PREVIEW_LIMIT` (100) rows of `table` (which may be schema-qualified e.g. "public.users").
 */
export async function GET(_req: Request, context: RouteContext) {
  try {
    ...
    console.error("Failed to fetch PostgreSQL table rows:", err);
    const message =
