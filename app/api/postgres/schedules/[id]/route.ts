...
async function loadOwnedPostgresSchedule(userId: string, id: string) {
  const [row] = await db
    .select({
...
/**
 * GET /api/postgres/schedules/[id]
 * Returns a single PostgreSQL schedule and its most recent runs.
 */
export async function GET(_req: Request, context: RouteContext) {
  try {
    ...
    const schedule = await loadOwnedPostgresSchedule(session.user.id, id);
    if (!schedule) {
      return NextResponse.json(
    ...
    const message =
      err instanceof Error ? err.message : "Failed to fetch PostgreSQL schedule.";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}

/**
 * PATCH /api/postgres/schedules/[id]
 * Partially updates a PostgreSQL schedule. Only enabled, cronExpression, timezone, and
...
export async function PATCH(req: Request, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    const existing = await loadOwnedPostgresSchedule(session.user.id, id);
    if (!existing) {
      return NextResponse.json(
        { error: "Schedule not found." },
        { status: 404 }
    ...
    console.error("PostgreSQL schedule update failed:", err);
    const message =
...
/**
 * DELETE /api/postgres/schedules/[id]
 * Deletes the PostgreSQL schedule. Cascades to backup_run rows.
 */
export async function DELETE(_req: Request, context: RouteContext) {
  try {
    ...
    console.error("PostgreSQL schedule deletion failed:", err);
    const message =
