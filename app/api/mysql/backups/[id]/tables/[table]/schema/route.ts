...
/**
 * GET /api/mysql/backups/[id]/tables/[table]/schema
 *
 * Streams the gzipped dump from S3, runs the parser, and returns the
 * matching `CREATE TABLE` SQL for `table`. Returns 404 when the table is
 * not present in the dump or when the backup is not owned by the caller.
 */
