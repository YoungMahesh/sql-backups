...
import { extractTableRows } from "@/lib/backup-parser";
...
/**
 * GET /api/mysql/backups/[id]/tables/[table]/rows
 *
 * Streams the gzipped dump from S3, runs the parser, and returns the first
 * `ROW_PREVIEW_LIMIT` rows of `table`. The row cap is hard-coded server-side;
 * the client cannot request more rows. Returns 404 when the table is not
 * present in the dump or when the backup is not owned by the caller.
 */
export async function GET(_req: Request, context: RouteContext) {
  try {
    ...
    const rows = await extractTableRows(stream, decodedTable, ROW_PREVIEW_LIMIT);
    if (rows === null) {
      return NextResponse.json(
    ...
    const message =
      err instanceof Error ? err.message : "Failed to fetch table rows.";
