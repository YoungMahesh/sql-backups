...
import { db } from "@/db";
import { backupRun, backupSchedule } from "@/db/schema";
...
async function loadOwnedSchedule(userId: string, id: string) {
  const [row] = await db
    .select()
    .from(backupSchedule)
    .where(
      and(eq(backupSchedule.id, id), eq(backupSchedule.userId, userId))
    )
    .limit(1);
  return row ?? null;
}
...
/**
 * GET /api/mysql/schedules/[id]
 * Returns a single schedule and its most recent runs.
 */
export async function GET(_req: Request, context: RouteContext) {
  try {
    ...
    const schedule = await loadOwnedSchedule(session.user.id, id);
    if (!schedule) {
      return NextResponse.json(
...
/**
 * PATCH /api/mysql/schedules/[id]
 * Partially updates a schedule. Only enabled, cronExpression, timezone, and
...
export async function PATCH(req: Request, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    const existing = await loadOwnedSchedule(session.user.id, id);
    if (!existing) {
      return NextResponse.json(
        { error: "Schedule not found." },
        { status: 404 }
...
/**
 * DELETE /api/mysql/schedules/[id]
 * Deletes the schedule. Cascades to backup_run rows. Existing database_backup
