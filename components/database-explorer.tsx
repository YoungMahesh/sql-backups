...
export interface SqliteTableItem {
  name: string;
  rowCount: number;
}
...
export function DatabaseExplorer({ onBackupCreated }: DatabaseExplorerProps = {}) {
  ...
  const [engine, setEngine] = useState<"mysql" | "postgres" | "sqlite">("mysql");
  ...
  // Dedicated SQLite / Turso fields
  const [sqliteUrl, setSqliteUrl] = useState("");
  const [sqliteAuthToken, setSqliteAuthToken] = useState("");
  const [showSqliteAuthToken, setShowSqliteAuthToken] = useState(false);
  const [sqliteDatabase, setSqliteDatabase] = useState("");
  const [sqliteTables, setSqliteTables] = useState<SqliteTableItem[]>([]);
  ...
  const handleEngineChange = (newEngine: "mysql" | "postgres" | "sqlite") => {
    ...
    } else if (newEngine === "postgres") {
      if (port === "3306" || port === "443" || !port) setPort("5432");
      if (user === "root" || user === "token" || !user) setUser("postgres");
      if (connectionString.startsWith("mysql://") || connectionString.startsWith("libsql://")) {
        setConnectionString("");
      }
    } else {
      if (port === "5432" || port === "443" || !port) setPort("3306");
      if (user === "postgres" || user === "token" || !user) setUser("root");
      if (
        connectionString.startsWith("postgresql://") ||
        connectionString.startsWith("postgres://") ||
  ...
  const handleTestConnection = async () => {
    ...
    try {
      let endpoint = "/api/mysql/databases";
      let payload: Record<string, unknown> = {};

      if (engine === "sqlite") {
        endpoint = "/api/sqlite/databases";
        payload =
          connectMode === "uri"
            ? {
                mode: "uri",
            ...
            : {
                mode: "params",
                url: sqliteUrl.trim(),
                authToken: sqliteAuthToken.trim(),
                database: sqliteDatabase.trim(),
                testOnly: true,
              };
      } else if (engine === "postgres") {
        endpoint = "/api/postgres/databases";
        payload =
          connectMode === "uri"
            ? {
                mode: "uri",
            ...
            : {
                mode: "params",
                host: host.trim(),
                port: port.trim() ? parseInt(port.trim(), 10) : 5432,
                user: user.trim(),
                password,
                testOnly: true,
              };
      } else {
        endpoint = "/api/mysql/databases";
        payload =
          connectMode === "uri"
            ? {
                mode: "uri",
  ...
  const handleConnect = async (e?: React.FormEvent) => {
    ...
    try {
      let endpoint = "/api/mysql/databases";
      let payload: ActiveConnectionState;
  ...
  const handleFillConnection = async (id: string) => {
    ...
    try {
      ...
      if (conn) {
        ...
        if (connEngine === "sqlite") {
          setSqliteUrl(conn.url || conn.connectionString || "");
          setSqliteAuthToken(conn.authToken || conn.password || "");
          setSqliteDatabase(conn.database || "");
        } else {
  ...
  const filteredSqliteTables = sqliteTables.filter((tbl) =>
    tbl.name.toLowerCase().includes(searchQuery.trim().toLowerCase())
  );

  const totalSqliteRows = sqliteTables.reduce((acc, tbl) => acc + (tbl.rowCount || 0), 0);
    ...
    <div className="space-y-6">
      ...
      {!isConnected ? (
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-hairline pb-5">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  {engine === "sqlite"
                    ? "Connect via Database URL & Auth Token or paste a full connection string."
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex rounded-lg border border-hairline bg-surface-soft p-1">
                  ...
                  >
                    {engine === "sqlite" ? "Database URL & Token" : "4 Server Fields"}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
              ...
              {connectMode === "params" ? (
                engine === "sqlite" ? (
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div className="sm:col-span-2">
                      ...
                      >
                        Database URL <span className="text-primary">*</span>
                      </label>
                      <div className="mt-1.5">
                        <input
                          ...
                          placeholder="libsql://<db-name>-<org>.turso.io or https://..."
                      ...
                      <p className="mt-1 text-[11px] text-muted-soft">
                        Remote libSQL or Turso endpoint URL
                      </p>
                    </div>
                    ...
                    <div className="sm:col-span-2 md:col-span-1">
                      ...
                      <div className="relative mt-1.5">
                        <input
                          id="sqlite-token"
                          type={showSqliteAuthToken ? "text" : "password"}
                          value={sqliteAuthToken}
                          onChange={(e) => setSqliteAuthToken(e.target.value)}
                          placeholder="Bearer token from Turso dashboard (e.g. eyJhbGci...)"
                          className="block w-full rounded-md border border-hairline bg-canvas px-3.5 py-2.5 pr-10 font-mono text-sm text-ink placeholder:text-muted-soft focus:border-primary focus:bg-canvas focus:outline-none focus:ring-2 focus:ring-primary/20"
                        />
                        <button
                          type="button"
                          onClick={() => setShowSqliteAuthToken(!showSqliteAuthToken)}
                          className="absolute inset-y-0 right-0 flex items-center pr-3 text-muted hover:text-ink"
                          tabIndex={-1}
                          aria-label={showSqliteAuthToken ? "Hide auth token" : "Show auth token"}
                        >
                          {showSqliteAuthToken ? (
                      ...
                      <p className="mt-1 text-[11px] text-muted-soft">
                        Bearer authentication token (optional for local/unsecured instances)
                      </p>
                    </div>
                    ...
                    <div className="sm:col-span-2 md:col-span-1">
                      ...
                      <div className="mt-1.5">
                        <input
                          ...
                          onChange={(e) => setSqliteDatabase(e.target.value)}
                          placeholder="Auto-derived from URL if left blank (e.g. my-app)"
                      ...
                      <p className="mt-1 text-[11px] text-muted-soft">
                        Derived from subdomain prefix if left empty
                      </p>
                    </div>
                  </div>
                ) : (
                ...
                <div>
                  <label
                    ...
                    className="block text-xs font-semibold uppercase tracking-wider text-body"
                  >
                    {engine === "sqlite" ? "SQLite (libSQL / Turso)" : engine === "postgres" ? "PostgreSQL" : "MySQL"} Connection String <span className="text-primary">*</span>
                  </label>
                  <div className="mt-1.5">
                    <input
                      ...
                      placeholder={
                        engine === "sqlite"
                          ? "libsql://<db-name>-<org>.turso.io?authToken=eyJ..."
                          : engine === "postgres"
                  ...
                  <p className="mt-1.5 text-xs text-muted">
                    ...
                    {engine === "sqlite"
                      ? "(authToken query parameter is preserved)"
            ...
            {isLoadingConnections && savedConnections.length === 0 ? (
              ...
              <div className="rounded-lg border border-dashed border-hairline py-8 text-center bg-canvas/40">
                ...
                <p className="mt-1 text-[11px] text-muted">
                  Whenever you connect to a MySQL, PostgreSQL, or SQLite (Turso) database above, it will be automatically saved here for one-click access.
                </p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          >
                            {item.engine === "sqlite" ? "SQLite" : item.engine === "postgres" ? "PostgreSQL" : "MySQL"}
                          </span>
        ...
        <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-semibold text-ink">
                    Connected to {serverInfo?.host}{serverInfo?.port ? `:${serverInfo?.port}` : ""}
                  </span>
                  <span
                ...
                <p className="mt-0.5 text-xs text-muted">
                  {activeConnection?.engine === "sqlite" ? (
                    ...
                    <>
                      Logged in as user:{" "}
                      <span className="font-mono font-medium text-ink">{serverInfo?.user}</span>
                    </>
                  )}
                </p>
              </div>
          ...
          {/* SQLite Table Explorer View */}
          {activeConnection?.engine === "sqlite" ? (
            <div className="rounded-xl border border-hairline bg-surface-card p-6 shadow-2xs">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-5 border-b border-hairline">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-serif text-lg font-normal text-ink">
                      Tables in {databases[0] || sqliteDatabase || "SQLite Database"}
                    </h3>
                    <p className="text-xs text-muted">
                      Total {sqliteTables.length} user table{sqliteTables.length === 1 ? "" : "s"} discovered • {totalSqliteRows.toLocaleString()} total rows across tables (system metadata excluded)
                    </p>
                  </div>
                </div>
                  ...
                  <div className="rounded-lg border border-dashed border-hairline py-12 text-center bg-canvas/40">
                    ...
                    <p className="mt-1 text-xs text-muted">
                      {searchQuery
                        ...
                        : "No user tables found in this SQLite database. (System metadata tables sqlite_*, _litestream*, _libsql* are excluded)"}
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {filteredSqliteTables.map((tbl) => (
                      <div
                        key={tbl.name}
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0 pr-2">
                          ...
                          <div className="min-w-0">
                            <p className="truncate font-mono text-sm font-semibold text-ink" title={tbl.name}>
                              {tbl.name}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <span className="inline-flex items-center rounded-md border border-hairline bg-surface-cream-strong px-2 py-0.5 font-mono text-xs font-medium text-body-strong">
